...
class easyViz{
    ...
    line(p,q,onlypath){
        ...
        const path=this.geo.geodesic(p,q,this.lineQuality);
        for(const p of path){
    ...
    arcStroke(center,radius,d){
        if(!d){
            d=32;
        }
        let past;
        for(let k=0; k<=d; ++k){
            const n=this.geo.translate(c128.poler(this.geo.pd(radius),2*Math.PI*k/d),center);
            if(past){
                this.line(past,n);
            }
            past=n;
